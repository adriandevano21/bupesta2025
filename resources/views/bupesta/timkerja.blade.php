...
<body>
    ...
    {{-- ======================================================= --}}
    {{-- AWAL BLOK OTORISASI (ROLE-BASED ACCESS CONTROL) --}}
    {{-- ======================================================= --}}
    @php
        // 1. Identifikasi Data User Login
        $userActive = $data['user_active'] ?? null;

        // NORMALISASI: Hapus spasi dan jadikan huruf kecil untuk akurasi pengecekan string
        $userRole = strtolower(trim($userActive->bupesta ?? ''));
        $userNip = trim($userActive->nip_pegawai ?? '');

        // FALLBACK: Kadang DB pakai huruf kecil 'kode_satker' atau 'kode_kab'
        $userSatker = trim($userActive->kode_Satker ?? ($userActive->kode_satker ?? ($userActive->kode_kab ?? '')));

        // 2. Pemetaan Role
        $isAdmin = $userRole === 'admin';
        $isKepalaUmum = $userRole === 'kepala-umum';

        // Validasi Role Pimpinan Daerah
        $isPimpinanDaerah = $userRole === 'kepala-kako' || $userRole === 'kasubbag';

        // 3. Hak Akses Fitur Global
        $canAddTim = $isAdmin || $isKepalaUmum;
        $canEditInfoUmum = $isAdmin;
    @endphp
    {{-- ======================================================= --}}
    ...
    <div id="page">
        ...
        <div class="konten">
            ...
            <div class="posisitengah">
                {{-- PERULANGAN GRUP TIM KERJA (STATUS 1) & AD HOC (STATUS 2) --}}
                @php
                    $timGroups = [
                        ['title' => 'Daftar Tim Kerja', 'status' => 1, 'showAddButton' => false],
                        ['title' => 'Daftar Tim Ad Hoc', 'status' => 2, 'showAddButton' => true],
                    ];
                @endphp

                @foreach ($timGroups as $index => $group)
                    <div class="header-aksi"
                        style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; flex-wrap: wrap; gap: 10px;">
                        <h2 style="margin: 0; font-size: 1.5rem; color: #f79039; font-weight: 600;">
                            <i class="fas fa-users-cog" style="margin-right: 8px;"></i> {{ $group['title'] }}
                        </h2>

                        {{-- Akses Tambah Tim (Admin & Kepala Umum) --}}
                        @if ($group['showAddButton'] && $canAddTim)
                    ...
                    <div class="container-kartu">
                        @foreach ($data['tim_kerja'] as $tim)
                            @if ($tim->status == $group['status'])
                                <div class="tiga-kartu">
                                    {{-- Script JSON Data Tim --}}
                                    <script type="application/json" id="data-tim-{{ $tim->kode_tim_kerja }}">{!! json_encode([
                                            'kode_tim_kerja' => $tim->kode_tim_kerja,
                                            'nama_tim_kerja' => $tim->nama_tim_kerja,
                                            'nip_ketua_tim'  => $tim->nip_ketua_tim,
                                            'ketuatim_1101'  => $tim->ketuatim_1101, 'ketuatim_1102'  => $tim->ketuatim_1102,
                                            'ketuatim_1103'  => $tim->ketuatim_1103, 'ketuatim_1104'  => $tim->ketuatim_1104,
                                            'ketuatim_1105'  => $tim->ketuatim_1105, 'ketuatim_1106'  => $tim->ketuatim_1106,
                                            'ketuatim_1107'  => $tim->ketuatim_1107, 'ketuatim_1108'  => $tim->ketuatim_1108,
                                            'ketuatim_1109'  => $tim->ketuatim_1109, 'ketuatim_1110'  => $tim->ketuatim_1110,
                                            'ketuatim_1111'  => $tim->ketuatim_1111, 'ketuatim_1112'  => $tim->ketuatim_1112,
                                            'ketuatim_1113'  => $tim->ketuatim_1113, 'ketuatim_1114'  => $tim->ketuatim_1114,
                                            'ketuatim_1115'  => $tim->ketuatim_1115, 'ketuatim_1116'  => $tim->ketuatim_1116,
                                            'ketuatim_1117'  => $tim->ketuatim_1117, 'ketuatim_1118'  => $tim->ketuatim_1118,
                                            'ketuatim_1171'  => $tim->ketuatim_1171, 'ketuatim_1172'  => $tim->ketuatim_1172,
                                            'ketuatim_1173'  => $tim->ketuatim_1173, 'ketuatim_1174'  => $tim->ketuatim_1174,
                                    ...
                                    {{-- Hak Akses Edit Kartu Tim (Global) --}}
                                    @if ($canAddTim || $userNip === trim($tim->nip_ketua_tim))
                                    ...
                                    <div class="isi-kartu">
                                        <div class="ketua">
                                            Ketua:
                                            @php $profilKetua = collect($data['pegawai_prov'])->firstWhere('nip_pegawai', $tim->nip_ketua_tim); @endphp
                                            @if ($profilKetua)
    ...
    <div id="modalKegiatanBesar" class="modal-overlay" style="display: none;">
        <div class="modal-content modal-xl">
            ...
            <div class="modal-tabs">
                ...
                <button class="tab-link {{ !$canEditInfoUmum ? 'pjk-butuh-cek-js' : '' }}"
                    style="display: {{ $canEditInfoUmum ? 'inline-block !important' : 'none' }};"
                ...
                <div id="tab-info" class="tab-content show">
                    ...
                    <div class="grup-tampil" style="margin-top: 15px;">
                        ...
                        <div class="teks-isian" id="teks_detail_anggota_tim"
                            style="display: flex; flex-wrap: wrap; gap: 5px; padding: 10px 0;">-</div>
                    </div>
                    <div class="grup-tampil">
                    ...
                    <div class="list-satker-pjk">
                        @foreach ($data['satkers'] as $satker)
                            <div class="baris-satker">
                                @php
                                    // Ekstraksi nilai loop dengan aman
                                    $loopSatker = trim($satker->kode_Satker ?? ($satker->kode_satker ?? ''));

                                    // Validasi Cocok (Pakai strict operator karena tipe data sudah dipastikan string oleh trim)
                                    $isSatkerSendiri = $userSatker !== '' && $userSatker === $loopSatker;

                                    // Hak Eksekusi
                                    $canEditDaerah = $isAdmin || ($isPimpinanDaerah && $isSatkerSendiri);
                                @endphp

                                <!-- DEBUG: UserRole='{{ $userRole }}', UserSatker='{{ $userSatker }}', LoopSatker='{{ $loopSatker }}', isPimpinan='{{ $isPimpinanDaerah }}', isSatkerSendiri='{{ $isSatkerSendiri }}' -->

                                <div class="aksi-kiri {{ !$canEditDaerah ? 'pjk-butuh-cek-js' : '' }}"
                                    style="display: {{ $canEditDaerah ? 'block !important' : 'none' }};">
                                    <button type="button" class="btn-edit-inline"
                                        onclick="toggleEditPjk('{{ $loopSatker }}')" title="Edit PJK">
                                ...
                                <div class="aksi-pjk" id="tampilan_pjk_{{ $loopSatker }}">
                                    <button type="button" class="btn-profil-pjk" id="btn_pjk_{{ $loopSatker }}"
                                        onclick="lihatProfilPegawai(this)">
                                        <i class="fas fa-user-circle"></i>
                                        <span class="teks-nama-pjk">Belum ada PJK</span>
                                    </button>
                                </div>

                                <div class="area-edit-pjk" id="area_edit_pjk_{{ $loopSatker }}"
                                    style="display: none;">
                                    <select id="select_pjk_{{ $loopSatker }}" class="input-form select-mini">
                                        <option value="">-- Kosongkan / Pilih PJK --</option>
                                        @foreach ($data['all_users'][$loopSatker] ?? [] as $pegawai)
                                            <option value="{{ $pegawai->nip_pegawai }}">{{ $pegawai->name }}</option>
                                        @endforeach
                                    </select>
                                    <button type="button" class="btn-simpan-mini"
                                        onclick="simpanPjk('{{ $loopSatker }}')" title="Simpan"><i
                                            class="fas fa-check"></i></button>
                                    <button type="button" class="btn-batal-mini"
                                        onclick="toggleEditPjk('{{ $loopSatker }}')" title="Batal"><i
    ...
    {{-- ======================================================= --}}
    {{-- MODAL TIM KERJA BESAR (TAB KETUA TIM DAERAH) --}}
    {{-- ======================================================= --}}
    <div id="modalTimKerjaBesar" class="modal-overlay" style="display: none;">
        <div class="modal-content modal-xl">
            ...
            <div class="modal-tabs">
                <button class="tab-link-tim active" onclick="openTabTim(event, 'tab-info-tim')"><i
                        class="fas fa-info-circle"></i> Informasi Umum</button>
                <button class="tab-link-tim" onclick="openTabTim(event, 'tab-ketua-kabkota')"><i
                        class="fas fa-users"></i> Ketua Tim Kab/Kota</button>
            </div>
                    ...
                    <div class="list-satker-pjk">
                        @foreach ($data['satkers'] as $satker)
                            @if ($satker->kode_Satker != '1100')
                                <div class="baris-satker">
                                    @php
                                        // 1. Ekstraksi nilai loop dengan aman
                                        $loopSatker = trim($satker->kode_Satker ?? ($satker->kode_satker ?? ''));

                                        // 2. Otorisasi Statis (Admin & Pimpinan Daerah Lokal)
                                        $isSatkerSendiri = $userSatker !== '' && $userSatker == $loopSatker;
                                        $canEditDaerah = $isAdmin || ($isPimpinanDaerah && $isSatkerSendiri);
                                    @endphp

                                    {{-- Class 'tim-butuh-cek-js' digunakan sebagai pengait (hook) untuk Javascript --}}
                                    <div class="aksi-kiri {{ !$canEditDaerah ? 'tim-butuh-cek-js' : '' }}"
                                        style="display: {{ $canEditDaerah ? 'block' : 'none' }};">
                                        <button type="button" class="btn-edit-inline"
                                            onclick="toggleEditKetuaTim('{{ $loopSatker }}')"
                                    ...
                                    <div class="aksi-pjk" id="tampilan_ketuatim_{{ $loopSatker }}">
                                        <button type="button" class="btn-profil-pjk"
                                            id="btn_ketuatim_{{ $loopSatker }}" onclick="lihatProfilPegawai(this)">
                                            <i class="fas fa-user-circle"></i>
                                            <span class="teks-nama-ketua">Belum ada Ketua</span>
                                        </button>
                                    </div>

                                    <div class="area-edit-pjk" id="area_edit_ketuatim_{{ $loopSatker }}"
                                        style="display: none;">
                                        <select id="select_ketuatim_{{ $loopSatker }}"
                                        ...
                                        <button type="button" class="btn-simpan-mini"
                                            onclick="simpanKetuaTim('{{ $loopSatker }}')" title="Simpan"><i
                                                class="fas fa-check"></i></button>
                                        <button type="button" class="btn-batal-mini"
                                            onclick="toggleEditKetuaTim('{{ $loopSatker }}')" title="Batal"><i
                                                class="fas fa-times"></i></button>
                                    </div>
                                </div>
                            @endif
